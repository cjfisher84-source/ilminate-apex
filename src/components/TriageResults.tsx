...
export default function TriageResults({ structured }: TriageResultsProps) {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      {/* Risk Score */}
      <Card sx={{ bgcolor: '#FFFFFF', border: '2px solid #E0E4E8' }}>
        <CardContent>
        ...
        <AccordionDetails>
          <List dense>
            {structured.checks.map((check, idx) => (
              <ListItem key={idx}>
                <CheckCircleIcon sx={{ color: '#10B981', mr: 1, fontSize: '1.2rem' }} />
      ...
      {structured.notes && (
        <Card sx={{ bgcolor: '#F9FAFB', border: '1px solid #E5E7EB' }}>
          <CardContent>
            ...
            <Typography variant="body2" sx={{ color: '#1F2937', whiteSpace: 'pre-wrap' }}>
              {structured.notes}
            </Typography>
          </CardContent>
        </Card>
